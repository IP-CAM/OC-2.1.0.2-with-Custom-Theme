...
class ControllerModuleTgKSproSettings extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(isset($_POST['button-active'])){
	            ...
	            $this->response->redirect($this->url->link('module/tg_kspro_settings&submit=true', 'token=' . $this->session->data['token'], 'SSL'));
            }
		}
			...
			if(isset($_POST['button-save'])){
				if(is_writable(DIR_CATALOG . 'view/theme/' . $this->config->get('config_template') . '/skins') && is_writable(DIR_CATALOG . 'view/theme/' . $this->config->get('config_template') . '/skins/store_'.$data['edit_skin_store']) && is_writable(DIR_CATALOG . 'view/theme/' . $this->config->get('config_template') . '/skins/store_'.$data['edit_skin_store'].'/'.$data['active_skin_edit'])) {
					...
					if(file_exists(DIR_CATALOG . 'view/theme/' . $this->config->get('config_template') . '/skins/store_'.$data['edit_skin_store'].'/') && filetype(DIR_CATALOG . 'view/theme/' . $this->config->get('config_template') . '/skins/store_'.$data['edit_skin_store'].'/') == 'dir' && file_exists(DIR_CATALOG . 'view/theme/' . $this->config->get('config_template') . '/skins/store_'.$data['edit_skin_store'].'/'.$data['active_skin_edit'].'')) {
						...
						$this->session->data['success'] = $this->language->get('text_success');
						$this->response->redirect($this->url->link('module/tg_kspro_settings&submit=true&skin_edit=' . $data['active_skin_edit'] . '', 'token=' . $this->session->data['token'], 'SSL'));
					}
				}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {	
			...
			if(isset($_POST['install_tg_kspro_all_module'])){
				...
				$this->session->data['success'] = $this->language->get('text_success');
				$this->response->redirect($this->url->link('module/tg_kspro_settings', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['action'] = $this->url->link('module/tg_kspro_settings', 'token=' . $this->session->data['token'], 'SSL');
		...
		//Send the output.
		$this->response->setOutput($this->load->view('module/tg_kspro_settings.tpl', $data));
	}
	...
	public function install() {
		...
		$this->model_setting_setting->editSettingValue('config', 'config_image_popup_width', 500);
		$this->model_setting_setting->editSettingValue('config', 'config_image_popup_height', 500);
		
		$this->model_setting_setting->editSettingValue('config', 'config_image_category_width', 170);	
		$this->model_setting_setting->editSettingValue('config', 'config_image_category_height', 170);	
		
		$this->model_setting_setting->editSettingValue('config', 'config_image_product_width', 170);	
		$this->model_setting_setting->editSettingValue('config', 'config_image_product_height', 170);	
		
		$this->model_setting_setting->editSettingValue('config', 'config_image_related_width', 170);
		$this->model_setting_setting->editSettingValue('config', 'config_image_related_height', 170);

		$this->model_setting_setting->editSettingValue('config', 'config_template', 'kingstorepro');	
